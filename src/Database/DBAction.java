...
//            tim them giao vien nua.
    ...
    public static Subject selectSubjectAdm(String idmonhoc, String mssv) {
        PreparedStatement ptmt;
        DBConnection db = new DBConnection();
        String sql = "select * from dangki,sinhvien,lop,monhoc where idlop = (?) and mssv = (?)  and monhoc.idmonhoc = lop.monhoc_idmonhoc\n"
    ...
    public static double getDiemTB(double diemqt, double diemck, double hesogiuaky, double hesocuoiky) {
        double a = (diemqt * hesogiuaky + diemck * hesocuoiky) * 1000;
        double b = Math.round(a);
        return b / 1000;
    }

    public static void updateAtAdmin(ArrayList<Subject> subj, ArrayList<CreditStudent> std, int idlop, double hesogiuaky, double hesocuoiky) {
        Connection conn = null;
        PreparedStatement ptmt = null;
        DBConnection db = new DBConnection();
        conn = db.ketNoi();
        ...
        try {
            ...
            for (int i = 0; i < subj.size(); i++) {
                if (idlop == subj.get(i).getCl().getIdClass()) {
                    midpoint = subj.get(i).getMidPoint();
                    endpoint = subj.get(i).getEndPoint();
                    type = subj.get(i).getType();
                    ptmt.setDouble(1, midpoint);
                    ptmt.setDouble(2, endpoint);
                    averpoint = DBAction.getDiemTB(subj.get(i).getMidPoint(), subj.get(i).getEndPoint(), hesogiuaky, hesocuoiky);
                    ptmt.setDouble(3, averpoint);
                    ptmt.setString(4, type);
                    ptmt.setInt(5, std.get(i).getIdStu());
                    ptmt.executeUpdate();
                }
            }
        } catch (Exception e) {
